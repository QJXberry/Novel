...
class Tree(object):
    ...
    def entropy(Y):
        ...
        # e = 0
        # for y in Y:
        #     e += -(Y.Counter(y)/len(Y)) * math.log(Y.Counter(y)/len(Y),2)
    ...
    def information_gain(Y,X):
        ...
        #########################################
        ## INSERT YOUR CODE HERE
        entropy = Tree.entropy(Y)
        conditional_entropy = Tree.conditional_entropy(Y, X)
        g = entropy- conditional_entropy
    ...
    def best_attribute(X,Y):
        ...
        for j in range(X.shape[0]):
            info_gain = Tree.information_gain(Y, X[j])
        ...
        for value in countX:
            ...
            for j in range(len(X[i])):
                if X[i, j] == value:
                    n = len(X[...,j])
                    if numX.size == 0:
                        numX = X[...,j].reshape(n,1)
                    else:
                        numX = np.hstack([numX, X[...,j].reshape(n,1)])
                        
                    numY = np.append(numY, Y[j])
            C[value] = Node(numX, numY)
    ...
    def inference(t,x):
        ...
        #     y = t.p
        # else:
        #     inference = Tree.inference(t.C[x[t.i]], x)
        #     y = inference

        if t.isleaf == False:
            attribute = x[t.i]
            if attribute in t.C:
                y = Tree.inference(t.C[attribute], x)
            else:
                y = t.p
        else:
            y = t.p
    ...
    def predict(t,X):
        ...
        #     inference = Tree.inference(t, X[:,i])
        #     Y.append(inference)
