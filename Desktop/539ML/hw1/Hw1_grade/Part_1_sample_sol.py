...
class Tree(object):
    ...
    def entropy(Y):
        ...
        for value in Counter(Y).values():
            e -= value * math.log2(value/length)
        e /= length
        #########################################
        return e 
    ...
    def conditional_entropy(Y,X):
        ...
        for x in x_list:
            sub_y = Y[X == x]
            e = Tree.entropy(sub_y)
            prob = 1.0 * len(sub_y)/len(Y)
            ce += e * prob    
        #########################################
        return ce 
    ...
    def information_gain(Y,X):
        ...
        #########################################
        ## INSERT YOUR CODE HERE
        base_e = Tree.entropy(Y)
        con_e = Tree.conditional_entropy(Y,X)
        g = base_e - con_e 
        #########################################
        return g
    ...
    def best_attribute(X,Y):
        ...
        i = 0
        gain = 0
        for temp_index in range(instances):
            temp_gain = Tree.information_gain(Y, X[temp_index])
            if temp_gain > gain:
                i = temp_index
                gain = temp_gain  
        #########################################
    ...
    def split(X,Y,i):
        ...
        C = {}
        attr_array = X[i, :]
        attr_values = set(attr_array)
        for attr_value in attr_values:
            value_index = (attr_array == attr_value)
            new_node = Node(X[:, value_index], Y[value_index])
            C[attr_value] = new_node
        #########################################
        return C
    ...
    def build_tree(t):
        ...
        # recursively build subtree on each child node
            for attr_value in t.C.keys():
                Tree.build_tree(t.C[attr_value]) 
        #########################################
    ...
    def inference(t,x):
        ...
        #########################################
        ## INSERT YOUR CODE HERE
        if t.isleaf == True or x[t.i] not in t.C:
            y = t.p
        else:
    ...
    def load_dataset(filename = 'data1.csv'):
        ...
        #########################################
        ## INSERT YOUR CODE HERE
        raw_dat = np.genfromtxt(fname='data1.csv', dtype=None, delimiter=',', skip_header=0, names=True, encoding=None)
        raw_dat =  np.array([list(v) for v in raw_dat])
        Y = raw_dat[:,0]; X = raw_dat[:,1:]
        Y = Y.reshape(len(Y),)
        X = X.T.reshape(len(X[0]),-1)
        #########################################
        return X,Y
