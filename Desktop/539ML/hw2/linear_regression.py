...
def compute_yhat(Phi, w):
    ...
    return np.squeeze(np.asarray(yhat))
...
def compute_dL_dw(y, yhat, Phi):
    ...
    dL_dw = np.zeros(Phi.shape[1])
    for i in range(Phi.shape[1]):
        for j in range(len(y)):
            y_pred = yhat[j, 0] if isinstance(yhat, np.matrix) else yhat[j]
            dL_dw[i] += Phi[j, i] * (y_pred - y[j])
