...
from constants import USER_WRITER


class WriterLoginHandler(RequestHandler):
    def data_received(self, chunk):
        pass

    def get(self,  *args, **kwargs):
        try:
            self.render('bk_writer/login.html')
        except Exception, e:
            error_sen("WriterLoginHandler get", e)
    ...
    def post(self,  *args, **kwargs):
        try:
            ...
            if not user:
                ...
                if result:
                    self.write(get_result(ret=0, msg="登录成功！", href='../../static/bk_writer/main.html'))

        except Exception, e:
            error_sen("WriterLoginHandler post", e)
